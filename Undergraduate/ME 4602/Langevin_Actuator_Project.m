...
%% Request User input
V0=1;%10^-3;%input('Enter Peak Voltage [V]: ');
...
%States are: 1=Vel 1, 2=Pos 1, 3=Vel 2, 4=Pos 2...
%Odd states are velocity
%Even states are Position
...
for time=1:TimeSteps
    ...
    for State_num=1:N_States
        if mod(State_num,2)~=0; %State number is odd, Velocity
            This_State=This_State+1;
            Forceing(time,State_num)=0;
        elseif mod(State_num,2)==0; %State number is even, Position
...
%Column number of [A] is the statenumber: odd=velocity, even=position
%row number of [A] is derivative of state number: odd=Accel., even=vel

Springs=Element_Spring; %elemental stiffness reformated
Springs(length(Springs)+1)=0; %no elements exists past the last node
...
%         if mod(column,2)~=0; %column is odd, State number is odd, Velocity
%             Node_Number=Node_Number+1;
%         end
%         
%         if column==1 || column == 2 || row==1 || row==2
%             %BC's make first node fixed in spaces, position and velocity
%             %are not influded by outside actions
%             A(row,column)=0;
%             
...
%             if column==row-1
%                 if Node_Number<=length(Springs)
%                     A(row,column)=-(Springs(Node_Number-1)+Springs(Node_Number))/Node_Mass(Node_Number);
%                 elseif Node_Number>length(Springs)
%                     A(row,column)=-(Springs(Node_Number-1)+0)/Node_Mass(Node_Number);
%                 end
%                 
%             elseif column==row-3
%                 A(row,column)=Springs(Node_Number-1)/Node_Mass(Node_Number);
%                 
%             elseif column==row+1;
%                 A(row,column)=Springs(Node_Number-1)/Node_Mass(Node_Number-1);
%                 
%             end
%             
%         end
%     end
% end
% clear element_number row column Springs
% 
% %Build [B] matrix
...
X_IC(3:(length(IC)+3))=[IC(1) 0 IC(2)];
